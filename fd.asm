...
DTS_PC720	.TEXT	"IBM PC 720KB 3.5\" FLOPPY$"
;
...
DTS_PC144	.TEXT	"IBM PC 1.44MB 3.5\" FLOPPY$"
;	
...
FCB_PC360	.DB	009H		; SECTOR COUNT
		...
		.DB	0DFH		; STEP RATE, IBM PS/2 CALLS FOR 3ms, 0DH = 3ms SRT, HEAD UNLOAD TIME
		.DB	004H		; HEAD LOAD TIME, IBM PS/2 CALLS FOR 15ms 08H = 16ms HUT
		.DB	DOR_PC360	; DEFAULT DOR VALUE
		.DB	001H		; CONTROL REGISTER VALUE
		.IF	(($ - MDB_PC360) != MDB_LEN)
		.ECHO	"*** FCB SIZE ERROR!!! ***\n"
		.ENDIF
DTL_PC360	.TEXT	"360KB $"
DTS_PC360	.TEXT	"IBM PC 360KB 5.25\" FLOPPY$"
;	
...
FCB_PC120	.DB	00FH		; SECTOR COUNT
		...
		.DB	0DFH		; STEP RATE, IBM PS/2 CALLS FOR 3ms, 0DH = 3ms SRT, HEAD UNLOAD TIME
		.DB	008H		; HEAD LOAD TIME, IBM PS/2 CALLS FOR 15ms 08H = 16ms HUT
		.DB	DOR_PC120	; DEFAULT DOR VALUE
		.DB	000H		; CONTROL REGISTER VALUE
		.IF	(($ - MDB_PC120) != MDB_LEN)
		.ECHO	"*** FCB SIZE ERROR!!! ***\n"
		.ENDIF
DTL_PC120	.TEXT	"1.2MB $"
DTS_PC120	.TEXT	"IBM PC 1.2MB 5.25\" FLOPPY$"
;	
...
FCB_PC111	.DB	00FH		; SECTOR COUNT
		...
		.DB	0DFH		; STEP RATE, IBM PS/2 CALLS FOR 3ms, 0DH = 3ms SRT, HEAD UNLOAD TIME
		.DB	008H		; HEAD LOAD TIME, IBM PS/2 CALLS FOR 15ms 08H = 16ms HUT
		.DB	DOR_PC111	; DEFAULT DOR VALUE
		.DB	000H		; CONTROL REGISTER VALUE
		.IF	(($ - MDB_PC111) != MDB_LEN)
		.ECHO	"*** FCB SIZE ERROR!!! ***\n"
		.ENDIF
DTL_PC111	.TEXT	"1.11MB$"
DTS_PC111	.TEXT	"IBM PC 1.11MB 8\" FLOPPY$"
;	
;===============================================================================
...
;	IN	A,(FDC_MSR)
	...
	RET
;
; DOR BITS (3AH)									3.5"		3.5"		5.25"		5.25"		8"
;											720KB		1.44MB		360KB		1.2MB		1.11MB
;											-----		-----		-----		-----		------
;D0 (2) D0 -> O0 (19) TC	-> I8272A  (16)  TC (TERMINAL COUNT)			N/A		N/A		N/A		N/A		N/A
;D1 (3) D1 -> O1 (18) /MOTOR	-> FLOPPY  (16)  /MOTOR (MOTOR CONTROL)			N/A		N/A		N/A		N/A		N/A
;D2 (4) D2 -> O2 (17) MINI	-> FDC9229 (3)   MINI					1		0		1		0		0
;D3 (5) D3 -> O3 (16) SETPS2	-> FDC9229 (19)  P2 (AMOUNT OF PRECOMPENSATION)		0		0		0		0		0
;D4 (6) D4 -> O4 (15) SETPS1	-> FDC9229 (18)  P1 (AMOUNT OF PRECOMPENSATION)		0		0		0		0		0
;D5 (7) D5 -> O5 (14) SETPS0	-> FDC9229 (17)  P0 (AMOUNT OF PRECOMPENSATION)		1		1		1		1		1
;D6 (8) D6 -> O6 (13) /REDWC	-> FLOPPY  (2)   /REDWC (DENSITY SELECT)		1		0		1		0		0?
;D7 (9) D7 -> O7 (12) /DSKCHG_L	-> J2      (1-2) /DC/RDY -> FLOPPY (34) /DC/RDY (?)	N/A		N/A		N/A		N/A		N/A
;
#IF (PLATFORM == PLT_DISKIO)
DOR_INIT	.EQU	10100010B	; INITIAL DEFAULT DOR VALUE
DOR_PC720	.EQU	11100110B	; DC/RDY=HI, /REDWC=LO=DD, PRECOMP=001, MINI=HI=250KHZ, MOTOR=HI=OFF, TC=LOW
DOR_PC144	.EQU	10100010B	; DC/RDY=HI, /REDWC=LO=HD, PRECOMP=001, MINI=LO=500KHZ, MOTOR=HI=OFF, TC=LOW
DOR_PC360	.EQU	11100110B	; DC/RDY=HI, /REDWC=LO=DD, PRECOMP=001, MINI=HI=250KHZ, MOTOR=HI=OFF, TC=LOW
DOR_PC120	.EQU	10100010B	; DC/RDY=HI, /REDWC=LO=HD, PRECOMP=001, MINI=LO=500KHZ, MOTOR=HI=OFF, TC=LOW
DOR_PC111	.EQU	10100010B	; DC/RDY=HI, /REDWC=LO=??, PRECOMP=001, MINI=LO=500KHZ, MOTOR=HI=OFF, TC=LOW
#ENDIF
#IF ((PLATFORM == PLT_ZETA) | (PLATFORM == PLT_DISKIO3))
; MOTOR AND DENSITY SELECT ARE INVERTED ON ZETA
DOR_INIT	.EQU	11100000B	; INITIAL DEFAULT DOR VALUE
DOR_PC720	.EQU	10100100B	; DC/RDY=HI, /REDWC=HI=DD, PRECOMP=001, MINI=HI=ON, MOTOR=LO=OFF, TC=LOW
DOR_PC144	.EQU	11100000B	; DC/RDY=HI, /REDWC=HI=DD, PRECOMP=001, MINI=LO=OFF, MOTOR=LO=OFF, TC=LOW
DOR_PC360	.EQU	10100100B	; DC/RDY=HI, /REDWC=HI=DD, PRECOMP=001, MINI=HI=ON, MOTOR=LO=OFF, TC=LOW
DOR_PC120	.EQU	11100000B	; DC/RDY=HI, /REDWC=HI=DD, PRECOMP=001, MINI=LO=OFF, MOTOR=LO=OFF, TC=LOW
DOR_PC111	.EQU	11100000B	; DC/RDY=HI, /REDWC=HI=DD, PRECOMP=001, MINI=LO=OFF, MOTOR=LO=OFF, TC=LOW
#ENDIF
#IF ((PLATFORM == PLT_DIDE) | (PLATFORM == PLT_N8))
DOR_INIT	.EQU	00001100B	; WITH DMAEN (REQUIRED)
DOR_PC720	.EQU	DOR_INIT	
DOR_PC144	.EQU	DOR_INIT
DOR_PC360	.EQU	DOR_INIT	
DOR_PC120	.EQU	DOR_INIT
DOR_PC111	.EQU	DOR_INIT
#ENDIF
;
;===============================================================================
